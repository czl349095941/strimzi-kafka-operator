...
class ConnectST extends AbstractST {
    ...
    void testKafkaConnectWithFileSinkPlugin() {
        ...
        TestUtils.waitFor("messages in file sink", GLOBAL_POLL_INTERVAL, TIMEOUT_FOR_SEND_RECEIVE_MSG,
            () -> kubeClient().execInPod(kafkaConnectPodName, kafkaConnectName(KAFKA_CLUSTER_NAME), "/bin/bash", "-c", "cat /tmp/test-file-sink.txt").equals("0\n1\n"));
    }
    ...
    void testKafkaConnectScaleUpScaleDown() {
        ...
        for (String pod : connectPods) {
            ...
            List<Event> events = kubeClient().listEvents("Pod", pod);
